...
- GPT-6 Astra（`gpt-6-astra`）按 2026-09-06 核验的[官方标准费率](https://developers.openai.com/api/docs/pricing)估算：每百万 Token 输入 $10、缓存读取 $1、缓存写入 $12.50、输出 $50，Sol 等价倍率为 2.5。当前报表缺少逐请求输入量和处理模式，未应用超过 272K 输入的长上下文倍率、Fast 倍率或 Batch/Flex 折扣，也未计入区域处理和工具调用附加费；缺失的缓存写入用量无法补算。这是 API 价格估算，不是订阅账单。
