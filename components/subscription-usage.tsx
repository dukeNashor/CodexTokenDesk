...
export function SubscriptionUsageCard({ usage, unit, detailsOnly = false }: { usage: SubscriptionUsage; unit: TokenUnit; detailsOnly?: boolean }) {
  if (detailsOnly) return <details className="subscription-disclosure"><summary><span>订阅消耗明细 <span className="estimate-tag">估算</span></span><small>{usage.models.length} 个模型 · 费率 {SUBSCRIPTION_RATE_CARD.checkedAt}{usage.models.some((model) => model.issues.length) ? " · 含缺失信息" : ""}</small></summary><SubscriptionUsageCard usage={usage} unit={unit} /></details>;
  return <section className="subscription-card" aria-label="订阅消耗估算">
    <header className="subscription-head">
      <div><h3>订阅消耗 <span className="estimate-tag">估算</span></h3><p>美元等值 · 全价估算 · {SUBSCRIPTION_RATE_CARD.checkedAt}</p></div>
      <LcdNumber value={formatCreditUsd(usage.estimatedCredits)} small />
    </header>
    <p className="subscription-basis">{SUBSCRIPTION_RATE_CARD.basis}</p>
    ...
    {usage.models.length > 0 ? <div className="subscription-models">{usage.models.map((model) => <article key={model.model} className="subscription-model">
      <div className="subscription-model-head"><strong>{model.model}</strong><span>{model.estimatedCredits === null ? "未定价" : <LcdNumber value={formatCreditUsd(model.estimatedCredits)} small />} {model.estimatedCredits !== null && <span className="estimate-tag">估算</span>}</span></div>
