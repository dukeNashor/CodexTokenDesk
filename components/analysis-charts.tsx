...
export function CumulativeChart({ turns, unit, onSelect }: { turns: AggregatedTurnReport[]; unit: TokenUnit; onSelect: (turn: AggregatedTurnReport) => void }) {
  ...
  return <div className="chart-card"><header className="chart-head"><div><h3>累计 Token 趋势</h3></div><LcdNumber value={formatTokens(running, unit)} small /></header><div className="chart-scroll"><svg className="turn-chart" viewBox={`0 0 ${width} ${height}`} style={{ minWidth: width }} role="img" aria-label="累计 Token 趋势图">
...
export function TurnHeatTable({ turns, unit, onSelect }: { turns: AggregatedTurnReport[]; unit: TokenUnit; onSelect: (turn: AggregatedTurnReport) => void }) {
  ...
  return <div className="chart-card detail-table-card"><header className="chart-head"><div><h3>逐轮明细</h3></div></header><div className="table-scroll"><table className="heat-table"><thead><tr><th>{heading("index", "轮次")}</th><th>来源</th><th>状态</th><th>{heading("total", "总 Token")}</th><th>{heading("cached", "缓存")}</th><th>普通输入</th><th>输出</th><th>{heading("context", "Context")}</th><th>{heading("tools", "工具")}</th><th>首条消息</th></tr></thead><tbody>{sorted.map((turn) => <tr key={`${turn.sourceRolloutId}-${turn.turnId}`} onClick={() => onSelect(turn)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(turn); } }}><td>#{turn.index}</td><td>{turn.sourceLabel}</td><td><LcdTag className={`status-tag ${turn.status}`}>{turn.status}</LcdTag></td><td><div className="heat-value"><span style={{ width: `${100 * turn.usage.total / maxTotal}%` }} /><b>{formatTokens(turn.usage.total, unit)}</b></div></td><td>{formatTokens(turn.breakdown.cachedInput, unit)}</td><td>{formatTokens(turn.breakdown.otherNonCachedInput, unit)}</td><td>{formatTokens(turn.breakdown.ordinaryOutput + turn.breakdown.reasoningOutput, unit)}</td><td>{turn.contextSnapshot.occupancyRate === null ? "—" : `${turn.contextSnapshot.occupancyRate.toFixed(1)}%`}</td><td>{turn.toolSummary.callCount || "—"}</td><td className="message-preview">{turn.messages[0]?.text || "—"}</td></tr>)}</tbody></table></div></div>;
}
